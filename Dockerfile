...
RUN apt-get update && apt-get install -y --no-install-recommends git make && \
    ...
    curl -fsSL https://archive.apache.org/dist/maven/maven-3/${MAVEN_VERSION}/binaries/apache-maven-${MAVEN_VERSION}-bin.tar.gz --output /tmp/apache-maven-${MAVEN_VERSION}-bin.tar.gz && \
    curl -fsSL https://archive.apache.org/dist/maven/maven-3/${MAVEN_VERSION}/binaries/apache-maven-${MAVEN_VERSION}-bin.tar.gz.sha512 --output /tmp/apache-maven-${MAVEN_VERSION}-bin.tar.gz.sha512 && \
